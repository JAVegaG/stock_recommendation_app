<template>
    <div
        ...
        <div class="flex items-center gap-3 order-1 sm:order-2">
            ...
            <!-- Botón Siguiente -->
            <BaseButton variant="secondary" size="sm" :disabled="currentPage >= totalPages || loading"
    ...
    <div v-if="totalPages > 5 && !isMobile" class="flex justify-center mt-4">
        <div class="flex items-center gap-1 bg-white rounded-lg shadow-sm border border-gray-200 p-2">
            ...
            <!-- Última página -->
            <button v-if="showLastPage" @click="goToSpecificPage(totalPages)" :disabled="loading"
                class="w-8 h-8 flex items-center justify-center text-sm font-medium rounded-md transition-colors hover:bg-gray-100 disabled:opacity-50"
                :class="currentPage === totalPages ? 'bg-blue-600 text-white hover:bg-blue-700' : 'text-gray-700'">
                {{ totalPages }}
            </button>
        </div>
    </div>
...
const showLastPage = computed(() => props.totalPages > 7 && !visiblePages.value.includes(props.totalPages))
const showLeftEllipsis = computed(() => showFirstPage.value && visiblePages.value[0] > 2)
const showRightEllipsis = computed(() => showLastPage.value && visiblePages.value[visiblePages.value.length - 1] < props.totalPages - 1)
